...
class FullClient(SocketHandler, Callbacks):
	...
	def join_chat(self, chatId: str, comId: Union[int, str] = None):
		if comId:
			response = self.req.make_request(method="POST", endpoint=f"/x{comId}/s/chat/thread/{chatId}/member/{self.profile.userId}", type="application/x-www-form-urlencoded", proxies=self.proxies, verify=self.certificatePath)
			return response.status_code		
	...
	def leave_chat(self, chatId: str, comId: Union[int, str] = None):
		if comId:
			response = self.req.make_request(method="DELETE", endpoint=f"{self.api}/x{comId}/s/chat/thread/{chatId}/member/{self.profile.userId}", proxies=self.proxies, verify=self.certificatePath)
			return response.status_code		
